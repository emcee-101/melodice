...
const OscillatorControls = ({ synthNum, synth }) => {
    ...
    return (<div className="control-container oscillator-container">
      ...
      <div className="row justify-center">
        <div className="waveform-container column">
          <div className="row justify-center">
            <RadioButtonGroup items={WAVEFORMS} id={`waveforms${synthNum}`} comparator={type} buttonSize="med" onValueChange={handleTypeChange}/>
          </div>
        </div>
      </div>
      <div className="row justify-center">
        <div className="volume-container">
          <div className="row justify-center">
            <div className="column hasTooltip">
              <Knob min={-60} max={0} value={volume} onValueChange={handleVolumeChange} width={50} height={50} step={1}/>
              <label className="unselectable title-small">Level</label>
              <span className="tooltip unselectable value">{`${Math.round(volume)}db`}</span>
            </div>
          </div>
        </div>
        <div className="phase-container">
          <div className="row justify-center">
            <div className="column hasTooltip">
              <Knob min={-180} max={180} value={phase} onValueChange={handlePhaseChange} width={50} height={50} step={1}/>
              <label className="unselectable title-small">Phase</label>
              <span className="tooltip unselectable value">{`${Math.round(phase)}°`}</span>
            </div>
          </div>
        </div>
        <div className="detune-container">
          <div className="row justify-center">
            <div className="column hasTooltip">
              <Knob min={-1200} max={1200} value={detune} onValueChange={handleDetuneChange} width={50} height={50} step={1}/>
              <label className="unselectable title-small">Detune</label>
              <span className="tooltip unselectable value">{`${Math.round(detune)}cents`}</span>
            </div>
          </div>
        </div>
      </div>
