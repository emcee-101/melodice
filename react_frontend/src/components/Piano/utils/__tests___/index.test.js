import { dbToY, getXofFrequency, keyIsPressed, keyIsSharp, valueToPercentage, percentageToValue, polarToCartesian, cartesianToPolar, getEndCoordinates, getBarCoordinates, } from "..";
...
describe("Control utils", () => {
    ...
    const height = 40, width = 40, radius = 20;
    ...
    test("getEndCoordinates returns {x: 20, y: 7} when: value is 100, min is 0, max is 200, minAngle and maxAngle are conjugate, radius is 20, circleX is 20, and circleY is 20", () => {
        expect(getEndCoordinates(100, min, max, 45, 315, radius, circleX, circleY)).toEqual({ x: 20, y: 7 });
    });
    test("getEndCoordinates returns {x: 7, y: 20} when: value is equal to min, minAngle is 90 and maxAngle is conjugate to minAngle, radius is 20, circleX is 20, and circleY is 20", () => {
        expect(getEndCoordinates(min, min, max, 90, 270, radius, circleX, circleY)).toEqual({ x: 7, y: 20 });
    });
    const barHeight = 20;
